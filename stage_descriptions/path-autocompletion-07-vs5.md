...
For example, if these entries exist your current working directory:
...
If there is only one match after performing completion, then the shell should complete the path as with previous stages (with a trailing space for filename and a trailing slash for a directory).
